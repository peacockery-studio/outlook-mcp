...
describe("TokenStorage", () => {
	...
	beforeEach(() => {
		...
		fs.unlink.mockReset();
		fs.writeFile.mockResolvedValue(undefined); // Default: successful write
		fs.unlink.mockResolvedValue(undefined); // Default: successful delete
	...
	describe("isTokenExpired", () => {
		...
		it("should return true if token is past expiration time (considering buffer)", () => {
			tokenStorage.tokens = {
				expires_at:
					Date.now() - (tokenStorage.config.refreshTokenBuffer + 1000),
		...
		it("should return false if token is not expired and outside buffer", () => {
			tokenStorage.tokens = {
				expires_at:
					Date.now() + (tokenStorage.config.refreshTokenBuffer + 60000),
	...
	describe("refreshAccessToken", () => {
		...
		it("should handle concurrent refresh calls by deduplicating requests", async () => {
			const promise1 = tokenStorage.refreshAccessToken();
			const promise2 = tokenStorage.refreshAccessToken();

			// Both should resolve to the same value (deduplication), though they may be different promise objects
			// due to .then() wrapping in the implementation
